...
const orderStatus = ["Pending", "Appproved", "PaymentFailed", "Completed"];
...
export default function OrderList() 
{
    ...
    const subTotal = selectedOrder?.orderItems.reduce((toplam, item) => toplam + (item.price * item.quantity), 0) ?? 0;
                            ...
                            <TableBody>
                                {selectedOrder?.orderItems.map((item) => (
                                    <TableRow key={item.id}>
                                        <TableCell>
                                            <img src={`http://localhost:5052/images/${item.productImage}`} 
                                            style={{height: 60}} />                                       
                                        </TableCell>    
